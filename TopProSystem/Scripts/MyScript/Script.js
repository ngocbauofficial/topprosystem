...
var expLoading = '<div id="exp-loading" style="position:fixed; left: 50%; bottom: 50%; display: block; "><span class="fa fa-spinner fa-pulse fa-3x fa-fw"></span></div>';
...
function ErrorAlert(message) {
    ...
    notify.fadeIn(200).delay(1000).fadeOut(200);
...
function openHiddenMenu(menu, open_button, left_size, background_shadow) {
    if (menu.css('left') == left_size) {
        menu.css('visibility','visible').css('left', '0');
...
function _closeHiddenMenu(menu, background_shadow, left_size, open_button) {
    menu.css('left', left_size).css('visibility', 'hidden');
