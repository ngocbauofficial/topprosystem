...
namespace TopProSystem.Areas.MasterSetting.DAL.PUR001
{
    public class PUR001_DAL
    {
        ...
        protected IQueryable<Models.PUR001> PUR001s()
        {
            return dc.PUR001.Where(x => x.Deleted != 1);
        }
        ...
        public bool Insert(Models.PUR001 model)
        {
            try
            {
                var modelChecking = PUR001s().FirstOrDefault(x => x.AAPURNO.Trim().Equals(model.AAPURNO));
        ...
        public bool Delete(string key, bool deleteForUpdate = false) // update thi xoa han, delete thi thay doi trang thai
        {
            ...
            try
            {
                ...
                {
                    var list = PUR001s().Where(x => x.AADSTNC.Trim().Equals(key));
        ...
        {
            var models = PUR001s().Where(x => x.AADSTNC.Equals(key) && x.AACMPCD.ToUpper().Equals("open")).FirstOrDefault(); // lấy ra thằng chưa complete
            if (models == null) // đã complete all
            {
                models = PUR001s().OrderByDescending(x => x.AACMPDT).First(x => x.AADSTNC.Equals(key)); // lay ra thang complete gan nhat
            }
            return models;
        }
     
        public Models.PUR001[] GetAllCompletedItem(string key)
        {
            Models.PUR001[] itemNo = null;
            var models = PUR001s().Where(x => x.AADSTNC.Equals(key) && x.CompletetionStatus == 1);
            if (models == null)
            {
                return itemNo;
            }

            itemNo = models.ToArray();
            return itemNo;
        }
        public IEnumerable<Models.PUR001> GetAll()
        {
            return PUR001s().GroupBy(x => x.AADSTNC, (e, g) => g.FirstOrDefault());
        }
        public List<Models.PUR001> GetAllItemOfPurchase(string key, bool reference = false)
        {
            var models = PUR001s().OrderByDescending(x => x.ABRGSDT).Where(x => x.AADSTNC.Equals(key)).ToList();
        ...
        {
             var model = from a in PUR001s() select a;
            ...
            {
                model = model.Where(x => x.AARMTP == searchmodel.RawMaterialType);
            }
        ...
        public bool CheckCompletionStatus(string privateKey) // check all item
        {
            var allItem = PUR001s().AsNoTracking().Where(x => x.AADSTNC.Trim().ToUpper().Equals(privateKey.Trim().ToUpper()));
            return allItem.All(x => x.CompletetionStatus == 1);
        }
        ...
        {
            return PUR001s().FirstOrDefault(x => x.AADSTNC.Trim().Equals(code)) == null;
        }
        public bool CheckSpNoExists(string a)
        {
            return PUR001s().FirstOrDefault(x => x.AAPURNO.Trim().Equals(a.Trim())) == null;
        }
        public string CreatePurchaseContractNo()
        {
            ...
            //   var model_nearest = dc.PUR001.OrderByDescending(x => x.AAPURNO).FirstOrDefault();  // lấy tất cả record
            var model_nearest_array = dc.PURNO_NEAREST_SINGLE().ToArray();  // store proc
            if (model_nearest_array.Length == 0)
            {
            ...
            {
                var model_nearest = model_nearest_array[0];
                //  purchr_ctract_no_character = model_nearest.AAPURNO.Substring(0, 4);
                purchr_ctract_no_character = model_nearest.Substring(0, 4);
                ...
                if (purchr_ctract_no_character.Equals(rs_code))  // hop dong trong thang
                {
                    ...
                    purchr_ctract_no_number = model_nearest.Substring(4, 4);
        ...
        public int GetTotalRecord(Dictionary<string, string> args, bool completed = false)
        {
            if (args.All(x => x.Value == ""))
            {
                if (completed == false)
                {
                    return PUR001s().Count();
                }
                else
                {
                    return PUR001s().Where(x => x.CompletetionStatus != 1).Count();
                }
            }

            List<Models.PUR001> a;

            if (completed == true && string.IsNullOrEmpty(args["AACMPCD"]))
            {
                a = PUR001s().Where(x => x.CompletetionStatus != 1).Select(x => x).ToList();
            }
            else
            {
                a = PUR001s().Select(x => x).ToList();
            }


            if (!string.IsNullOrEmpty(args["AACMPCD"]))
            {
                if (args["AACMPCD"].Trim().ToLower().Equals("open"))
                {
                    a = a.Where(x => x.CompletetionStatus == 0).ToList();
                }
                else
                {
                    a = a.Where(x => x.CompletetionStatus == 1).ToList();
                }
            }
        ...
        public IEnumerable<Models.PUR001> GetTotalDisplayRecord(Dictionary<string, string> args, int iDisplayStart, int iDisplayLength, bool paging = true, bool completed = false)
        {
            ...
            if (args.All(x => x.Value == ""))
            {
                if (completed == false)
                {
                    if (!paging)
                    {
                        ieRs = PUR001s().OrderByDescending(x => x.ABRGSDT);
                    }
                    else
                    {
                        ieRs = PUR001s().OrderByDescending(x => x.ABRGSDT).Skip(iDisplayStart).Take(iDisplayLength);
                    }
                }
                else
                {
                    if (!paging)
                    {
                        ieRs = PUR001s().Where(x => x.CompletetionStatus != 1).OrderByDescending(x => x.ABRGSDT);
                    }
                    else
                    {
                        ieRs = PUR001s().Where(x => x.CompletetionStatus != 1).OrderByDescending(x => x.ABRGSDT).Skip(iDisplayStart).Take(iDisplayLength);
                    }
                }
        ...
        public bool ChangeCompletionStatusOfItem(string itemPrivateKey)
        {
            ...
            var item = PUR001s().Where(x => x.PURCODE.Value.Equals(guid)).SingleOrDefault();
            if (item != null)
            {
        ...
        public List<Models.PUR001> GetPurchaseOrderFromTo(string purchaseFr, string purchaseTo, bool takeFirst = true)
        {
            ...
            for (int i = from; i <= to; i++)
            {
                ...
                if (takeFirst)
                {
                    var model = PUR001s().Where(x => x.AAPURNO.Equals(key)).FirstOrDefault();
                    if (model != null)
                    {
                        rsList.Add(model);
                    }
                ...
                {
                    var model = PUR001s().Where(x => x.AAPURNO.Equals(key));
                    if (model != null)
                    {
        ...
        //public string GetExchangeRatetypeCodeByExchangeRate(string curcode, DateTime date) // su dung cho hien thi luc update
        //{
        //    double ddate = double.Parse(string.Concat(date.Month, date.Year));
        //    var model = dc.MA009.Where(x => x.MJCRRCD.Trim().Equals(curcode.Trim()) && x.MJEXRTD - ddate == 0 && x.MJEXRTT.Equals(exchangerateType)).FirstOrDefault();
        //    if (model != null)
        //    {
        ...
        {
            var models = PUR001s().FirstOrDefault(x => x.AAPURNO == PurchaseContractNo && x.ABCTITM == itemno);
            if (models != null)
                return models;
            return null;
        }
        ...
        public IEnumerable<Models.PUR001> GetPUR001sByStockentryDateAndVesselName(DateTime stockEntry, string vessName)
        {
            return PUR001s().Where(x => x.ABSEDT.Value.Date == stockEntry.Date && x.ABVESEL.Equals(vessName));
        }
    }
}