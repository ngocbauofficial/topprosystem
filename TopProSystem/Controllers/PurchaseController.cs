    ...
    public class PurchaseController : BasePurchaseController
    {
        ...
        public JsonResult ExportPurchaseOrderBalanceEnquiryToExcel(TypeOfOrderCompletionOrEnquiry type, bool completed = false) // ham nay in 2 loai file nhu tren enum
        {
            try
            {
                ...
                {
                    dataDisplayNoPaging = dal.GetTotalDisplayRecord(TempdSearch(), 0, 0, paging: false, completed: true); // lay du lieu hien thi khong phan trang da complete
                }

                if (dataDisplayNoPaging.Count() == 0)
                {
                    return Json(new { status = ExportFileResult.Nodata }, JsonRequestBehavior.AllowGet);
                }
                ...
                int subColumn = COLUMN, subRow = ROW;
                foreach (var _title in header)
                {
                    using (var range = workSheet.Cells[ROW, subColumn])
                    {
                        range.Value = _title;
                ...
                foreach (var array in listData)
                {
                    ...
                    {
                        using (var range = workSheet.Cells[subRow, subColumn])
                        {
                            if (val == null)
                            {
                                range.Value = "";
                ...
                ExcelFile.Save();
                return Json(new { status = ExportFileResult.Success, path = path }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                return Json(new { status = ExportFileResult.Fail, messages = ex.Message }, JsonRequestBehavior.AllowGet);
            }
        ...
        public enum PrintPurchaseOrderResult : int
        {
            Success = 1,
            Fail = 2,
            NoPrinter = 3
        ...
        public ActionResult PrintPurchaseOrder(string _array)
        {
            ...
            try
            {
                ...
                return Json(new { status = PrintPurchaseOrderResult.Success, path = paths.ToArray() }, JsonRequestBehavior.AllowGet);
                
            }
            catch (Exception ex)
            {

                return Json(new { status = PrintPurchaseOrderResult.Fail , message = ex.Message}, JsonRequestBehavior.AllowGet);
            }
        ...
        public string CreatePurchaseOrDerFile(TopProSystem.Areas.MasterSetting.Models.PUR001 data, int n)
        {
            ...
            int itemSpacing_top = 60 /*khoang cua item vs phan dac ta phia tren */, start_pos_number = 530 /*vi tri bat dau fill du lieu trong table */, left_start_pos = 45 /*vi tri bat dau ben trai bo di item no*/;
            BaseFont baseFont = BaseFont.CreateFont(Environment.GetEnvironmentVariable("SystemRoot") + "\\fonts\\Calibril.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED);
            BaseFont VietFont = BaseFont.CreateFont(Environment.GetEnvironmentVariable("SystemRoot") + "\\fonts\\Calibril.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED);
            BaseFont ChinaFont = BaseFont.CreateFont(Server.MapPath("~/Font/simhei.ttf"), BaseFont.IDENTITY_H, BaseFont.EMBEDDED);
            for (int i = 1; i <= pageNumber; i++)
            {
                if (i > 1)
                {
                    doc.NewPage();
                    start_pos_number = 530;
                }
                // add template
                PdfContentByte canvas = writer.DirectContentUnder;
                ...
                CreateBeginText(writer, baseFont, fsdata, 22, 170, data.AARMK1, chinaBaseFont: ChinaFont, chinaLg: true);
                CreateBeginText(writer, baseFont, fsdata, 22, 170 - spacing_top, data.AARMK2, chinaBaseFont: ChinaFont, chinaLg: true);
                CreateBeginText(writer, baseFont, fsdata, 22, 170 - spacing_top - spacing_top, data.AARMK3, chinaBaseFont: ChinaFont, chinaLg: true);
                CreateBeginText(writer, baseFont, fsdata, 22, 170 - spacing_top - spacing_top - spacing_top, data.AARMK4, chinaBaseFont: ChinaFont, chinaLg: true);
                CreateBeginText(writer, baseFont, fsdata, 22, 170 - spacing_top - spacing_top - spacing_top - spacing_top, data.AARMK5, chinaBaseFont: ChinaFont, chinaLg: true);
        ...
        public void CreateBeginText(PdfWriter writer, BaseFont latinbaseFont, int fontSize, float x, float y, string text, bool align = false, int alignment = Element.ALIGN_LEFT, BaseFont chinaBaseFont = null, bool chinaLg = false)
        {
            if (!string.IsNullOrEmpty(text))
            {
                writer.DirectContent.BeginText();
                writer.DirectContent.MoveText(x, y);
                if (!Regex.IsMatch(text, @"\p{IsLatin-1Supplement}") && chinaLg == true)
                {
                    writer.DirectContent.SetFontAndSize(chinaBaseFont, fontSize);
                }
                else
                {
                    writer.DirectContent.SetFontAndSize(latinbaseFont, fontSize);
                }
