...
namespace TopProSystem.Areas.MasterSetting.DAL.MA009
{
    public class MA009_DAL
    {
        ...
        public bool Insert(Models.MA009 model)
        {
            ...
            bool check = db.MA009.Count(x => x.MJCRRCD.Trim() == model.MJCRRCD.Trim() && x.MJEXRTD == model.MJEXRTD) > 0;
            if (check == false)
            {
                try
                {
                    ...
                    if (db.SaveChanges() > 0)
                    {
                        var _model = db.MA009.Where(x => x.MJCRRCD == model.MJCRRCD && x.MJEXRTD == model.MJEXRTD).SingleOrDefault();
                        if (_model != null)
                        {
                            return true;
                        }
        ...
        public bool Update(Models.MA009 model)
        {
            try
            {
                var _model = db.MA009.Where(x => x.ID == model.ID).SingleOrDefault();
                if (_model != null)
                {
        ...
        public bool Delete(int id)
        {
            try
            {
                var _model = db.MA009.Where(x => x.ID == id).SingleOrDefault();
                if (_model != null)
                {
                    db.MA009.Remove(_model);
                    if (db.SaveChanges() > 0)
                    {
                        if (db.MA009.Where(x => x.ID == id).SingleOrDefault() == null)
                            return true;
                    }
                }
            }
        ...
        public bool CheckCurrencyCodeInUsed(string currencyCode, double? date)
        {
            var _model = db.MA009.Where(x => x.MJCRRCD.Trim().Equals(currencyCode.Trim()) && x.MJEXRTD == date).SingleOrDefault();
            return _model == null;
        }


        public bool CheckCurrencyCodeInUsed(string currencyCode, double? date, int Id)
        {
            var _model = db.MA009.Where(x => x.ID == Id).SingleOrDefault();
            if (_model.MJEXRTD == date)
            {
                return true;
            }
            else
            {
                var model = db.MA009.Where(x => x.MJCRRCD.Trim().Equals(currencyCode.Trim()) && x.MJEXRTD == date).SingleOrDefault();
                return model == null;
            }
        ...
        public Models.MA009 GetMA009(int id)
        {
            var _model = db.MA009.Where(x => x.ID == id).SingleOrDefault();
            return _model;
        }
