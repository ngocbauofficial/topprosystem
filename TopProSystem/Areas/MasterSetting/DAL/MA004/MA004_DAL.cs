...
namespace TopProSystem.Areas.MasterSetting.DAL.MA004
{
    public class MA004_DAL
    {
        ...
        public IEnumerable<Models.MA004> GetAll()
        {
            return dc.MA004.ToList();
        }
        public Models.MA004 GetReference(Models.MA004 mA004)
        {
            ...
            mA004.WarehouseCategory = dc.MA012.OrderByDescending(x => x.MNRGSDT).Where(x => x.MNCLSCD.Equals(Models.ClassificationCode.CLASSIFICATTIONCODE035)).Select(x => new System.Web.Mvc.SelectListItem { Value = x.MNSRCD, Text = x.MNSRCD + " - " + x.MNSRNM });
            return mA004;
        }
    }
}